...
/**
 * @title 0xlend's xtoken Contract
...
contract xtoken is xtokenInterface, Exponential, TokenErrorReporter {
     ...
     */
    function initialize(xtrollerInterface xtroller_,
        ...
        // Set the xtroller
        uint err = _setxtroller(xtroller_);
        require(err == uint(Error.NO_ERROR), "setting xtroller failed");

        // Initialize block number and borrow index (block number mocks depend on xtroller being set)
    ...
    function transferTokens(address spender, address src, address dst, uint tokens) internal returns (uint) {
        ...
        uint allowed = xtroller.transferAllowed(address(this), src, dst, tokens);
        if (allowed != 0) {
            return failOpaque(Error.xtroller_REJECTION, FailureInfo.TRANSFER_xtroller_REJECTION, allowed);
        }
        ...
        // unused function
        // xtroller.transferVerify(address(this), src, dst, tokens);
     ...
     * @dev This is used by xtroller to more efficiently perform liquidity checks.
     * @param account Address of the account to snapshot
     * @return (possible error, token balance, borrow balance, exchange rate mantissa)
     */
    function getAccountSnapshot(address account) external view returns (uint, uint, uint, uint) {
        uint xtokenBalance = accountTokens[account];
        uint borrowBalance;
        ...
        return (uint(Error.NO_ERROR), xtokenBalance, borrowBalance, exchangeRateMantissa);
    }
    ...
    function mintFresh(address minter, uint mintAmount) internal returns (uint, uint) {
        ...
        uint allowed = xtroller.mintAllowed(address(this), minter, mintAmount);
        if (allowed != 0) {
            return (failOpaque(Error.xtroller_REJECTION, FailureInfo.MINT_xtroller_REJECTION, allowed), 0);
        }
        ...
        // unused function
        // xtroller.mintVerify(address(this), minter, vars.actualMintAmount, vars.mintTokens);
    ...
    function redeemFresh(address payable redeemer, uint redeemTokensIn, uint redeemAmountIn) internal returns (uint) {
        ...
        uint allowed = xtroller.redeemAllowed(address(this), redeemer, vars.redeemTokens);
        if (allowed != 0) {
            return failOpaque(Error.xtroller_REJECTION, FailureInfo.REDEEM_xtroller_REJECTION, allowed);
        }
        ...
        /* We call the defense hook */
        xtroller.redeemVerify(address(this), redeemer, vars.redeemAmount, vars.redeemTokens);
    ...
    function borrowFresh(address payable borrower, uint borrowAmount) internal returns (uint) {
        ...
        uint allowed = xtroller.borrowAllowed(address(this), borrower, borrowAmount);
        if (allowed != 0) {
            return failOpaque(Error.xtroller_REJECTION, FailureInfo.BORROW_xtroller_REJECTION, allowed);
        }
        ...
        /* We call the defense hook */
        // unused function
        // xtroller.borrowVerify(address(this), borrower, borrowAmount);
    ...
    function repayBorrowFresh(address payer, address borrower, uint repayAmount) internal returns (uint, uint) {
        ...
        uint allowed = xtroller.repayBorrowAllowed(address(this), payer, borrower, repayAmount);
        if (allowed != 0) {
            return (failOpaque(Error.xtroller_REJECTION, FailureInfo.REPAY_BORROW_xtroller_REJECTION, allowed), 0);
        }
        ...
        /* We call the defense hook */
        // unused function
        // xtroller.repayBorrowVerify(address(this), payer, borrower, vars.actualRepayAmount, vars.borrowerIndex);
     ...
     * @return (uint, uint) An error code (0=success, otherwise a failure, see ErrorReporter.sol), and the actual repayment amount.
     */
    function liquidateBorrowInternal(address borrower, uint repayAmount, xtokenInterface xtokenCollateral) internal nonReentrant returns (uint, uint) {
        uint error = accrueInterest();
        if (error != uint(Error.NO_ERROR)) {
     ...
     * @return (uint, uint) An error code (0=success, otherwise a failure, see ErrorReporter.sol), and the actual repayment amount.
     */
    function liquidateBorrowFresh(address liquidator, address borrower, uint repayAmount, xtokenInterface xtokenCollateral) internal returns (uint, uint) {
        /* Fail if liquidate not allowed */
        uint allowed = xtroller.liquidateBorrowAllowed(address(this), address(xtokenCollateral), liquidator, borrower, repayAmount);
        if (allowed != 0) {
            return (failOpaque(Error.xtroller_REJECTION, FailureInfo.LIQUIDATE_xtroller_REJECTION, allowed), 0);
        }
        ...
        (uint amountSeizeError, uint seizeTokens) = xtroller.liquidateCalculateSeizeTokens(address(this), address(xtokenCollateral), actualRepayAmount);
        require(amountSeizeError == uint(Error.NO_ERROR), "LIQUIDATE_xtroller_CALCULATE_AMOUNT_SEIZE_FAILED");
        ...
        /* We call the defense hook */
        // unused function
        // xtroller.liquidateBorrowVerify(address(this), address(xtokenCollateral), liquidator, borrower, actualRepayAmount, seizeTokens);
    ...
    function seizeInternal(address seizerToken, address liquidator, address borrower, uint seizeTokens) internal returns (uint) {
        ...
        uint allowed = xtroller.seizeAllowed(address(this), seizerToken, liquidator, borrower, seizeTokens);
        if (allowed != 0) {
            return failOpaque(Error.xtroller_REJECTION, FailureInfo.LIQUIDATE_SEIZE_xtroller_REJECTION, allowed);
        }
        ...
        /* We call the defense hook */
        // unused function
        // xtroller.seizeVerify(address(this), seizerToken, liquidator, borrower, seizeTokens);
    ...
    /**
      * @notice Sets a new xtroller for the market
      * @dev Admin function to set a new xtroller
      * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
      */
    function _setxtroller(xtrollerInterface newxtroller) public returns (uint) {
        // Check caller is admin
        if (msg.sender != admin) {
            return fail(Error.UNAUTHORIZED, FailureInfo.SET_xtroller_OWNER_CHECK);
        }

        xtrollerInterface oldxtroller = xtroller;
        // Ensure invoke xtroller.isxtroller() returns true
        require(newxtroller.isxtroller(), "marker method returned false");

        // Set market's xtroller to newxtroller
        xtroller = newxtroller;

        // Emit Newxtroller(oldxtroller, newxtroller)
        emit Newxtroller(oldxtroller, newxtroller);

...
/**
 * @title 0xlend's xkcc20Delegator Contract
 * @notice xtokens which wrap an EIP-20 underlying and delegate to an implementation
 * @author 0xlend
 */
contract xkcc20Delegator is xtokenInterface, xkcc20Interface, CDelegatorInterface {
    /**
    ...
    function _setImplementation(address implementation_, bool allowResign, bytes memory becomeImplementationData) public {
        require(msg.sender == admin, "xkcc20Delegator::_setImplementation: Caller must be admin");
     ...
     * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
     */
    function liquidateBorrow(address borrower, uint repayAmount, xtokenInterface xtokenCollateral) external returns (uint) {
        bytes memory data = delegateToImplementation(abi.encodeWithSignature("liquidateBorrow(address,uint256,address)", borrower, repayAmount, xtokenCollateral));
        return abi.decode(data, (uint));
    }
   ...
   /**
    ...
    function _setxtroller(xtrollerInterface newxtroller) public returns (uint) {
        bytes memory data = delegateToImplementation(abi.encodeWithSignature("_setxtroller(address)", newxtroller));
        return abi.decode(data, (uint));
    }
    ...
    function () external payable {
        require(msg.value == 0,"xkcc20Delegator:fallback: cannot send value to fallback");

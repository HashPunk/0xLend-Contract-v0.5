...
describe('Spinarama', () => {
  ...
  describe('#mintMint', () => {
    it('should succeed', async () => {
      ...
      await send(xtoken.underlying, 'harnessSetBalance', [from, 100], {from});
      await send(xtoken.underlying, 'approve', [xtoken._address, UInt256Max()], {from});
      await minerStop();
      const p1 = send(xtoken, 'mint', [1], {from});
      const p2 = send(xtoken, 'mint', [2], {from});
      await minerStart();
      expect(await p1).toSucceed();
      expect(await p2).toSucceed();
      expect(await balanceOf(xtoken, from)).toEqualNumber(3);
    });

    it('should partial succeed', async () => {
      const xtoken = await makextoken({supportMarket: true});
      await send(xtoken.underlying, 'harnessSetBalance', [from, 100], {from});
      await send(xtoken.underlying, 'approve', [xtoken._address, 10], {from});
      await minerStop();
      const p1 = send(xtoken, 'mint', [11], {from});
      const p2 = send(xtoken, 'mint', [10], {from});
      ...
      expect(await balanceOf(xtoken, from)).toEqualNumber(10);
    });
  });
  ...
  describe('#mintRedeem', () => {
    it('should succeed', async () => {
      ...
      await send(xtoken.underlying, 'approve', [xtoken._address, 10], {from});
      await minerStop();
      const p1 = send(xtoken, 'mint', [10], {from});
      const p2 = send(xtoken, 'redeemUnderlying', [10], {from});
      await minerStart();
      expect(await p1).toSucceed();
      expect(await p2).toSucceed();
      expect(await balanceOf(xtoken, from)).toEqualNumber(0);
    });
  });
  ...
  describe('#redeemMint', () => {
    it('should succeed', async () => {
      const xtoken = await makextoken({supportMarket: true});
      await send(xtoken, 'harnessSetTotalSupply', [10]);
      await send(xtoken, 'harnessSetExchangeRate', [etherMantissa(1)]);
      await send(xtoken, 'harnessSetBalance', [from, 10]);
      await send(xtoken.underlying, 'harnessSetBalance', [xtoken._address, 10]);
      await send(xtoken.underlying, 'approve', [xtoken._address, 10], {from});
      await minerStop();
      const p1 = send(xtoken, 'redeem', [10], {from});
      const p2 = send(xtoken, 'mint', [10], {from});
      await minerStart();
      expect(await p1).toSucceed();
      expect(await p2).toSucceed();
  ...
  describe('#repayRepay', () => {
    it('should succeed', async () => {
      const xtoken1 = await makextoken({supportMarket: true, underlyingPrice: 1, collateralFactor: .5});
      const xtoken2 = await makextoken({supportMarket: true, underlyingPrice: 1, xtroller: xtoken1.xtroller});
      await send(xtoken1.underlying, 'harnessSetBalance', [from, 10]);
      await send(xtoken1.underlying, 'approve', [xtoken1._address, 10], {from});
      await send(xtoken2.underlying, 'harnessSetBalance', [xtoken2._address, 10]);
      await send(xtoken2, 'harnessSetTotalSupply', [100]);
      await send(xtoken2.underlying, 'approve', [xtoken2._address, 10], {from});
      await send(xtoken2, 'harnessSetExchangeRate', [etherMantissa(1)]);
      expect(await enterMarkets([xtoken1, xtoken2], from)).toSucceed();
      expect(await send(xtoken1, 'mint', [10], {from})).toSucceed();
      expect(await send(xtoken2, 'borrow', [2], {from})).toSucceed();
      await minerStop();
      const p1 = send(xtoken2, 'repayBorrow', [1], {from});
      const p2 = send(xtoken2, 'repayBorrow', [1], {from});
      await minerStart();
      expect(await p1).toSucceed();
      expect(await p2).toSucceed();
      expect((await borrowSnapshot(xtoken2, from)).principal).toEqualNumber(0);
    });

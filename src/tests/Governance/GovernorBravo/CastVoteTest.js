...
async function enfranchise(led, actor, amount) {
  await send(led, 'transfer', [actor, etherMantissa(amount)]);
  await send(led, 'delegate', [actor], { from: actor });
}

describe("governorBravo#castVote/2", () => {
  let led, gov, root, a1, accounts, govDelegate;
  ...
  beforeAll(async () => {
    ...
    gov = await deploy('GovernorBravoDelegator', [address(0), led._address, root, govDelegate._address, 17280, 1, "100000000000000000000000"]);
  ...
  describe("Otherwise", () => {
    ...
    describe("and we take the balance returned by GetPriorVotes for the given sender and the proposal's start block, which may be zero,", () => {
      ...
      it("and we add that ForVotes", async () => {
        actor = accounts[1];
        await enfranchise(led, actor, 400001);
    ...
    describe('castVoteBySig', () => {
      ...
      it('casts vote on behalf of the signatory', async () => {
        await enfranchise(led, a1, 400001);
     ...
     it("receipt uses two loads", async () => {
      ...
      await enfranchise(led, actor, 400001);
      await enfranchise(led, actor2, 400001);
      await send(gov, 'propose', [targets, values, signatures, callDatas, "do nothing"], { from: actor });
      proposalId = await call(gov, 'latestProposalIds', [actor]);

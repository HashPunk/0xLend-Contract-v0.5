...
describe('GovernorBravo#queue/1', () => {
  ...
  describe("overlapping actions", () => {
    it("reverts on queueing overlapping actions in same proposal", async () => {
      ...
      const led = await deploy('led', [root]);
      const gov = await deploy('GovernorBravoImmutable', [timelock._address, led._address, root, 17280, 1, "100000000000000000000000"]);
      await send(gov, '_initiate');
      const txAdmin = await send(timelock, 'harnessSetAdmin', [gov._address]);

      await enfranchise(led, a1, 3e6);
      await mineBlock();

      const targets = [led._address, led._address];
      const values = ["0", "0"];
    ...
    it("reverts on queueing overlapping actions in different proposals, works if waiting", async () => {
      ...
      await enfranchise(led, a1, 3e6);
      await enfranchise(led, a2, 3e6);
      await mineBlock();

      const targets = [led._address];
      const values = ["0"];

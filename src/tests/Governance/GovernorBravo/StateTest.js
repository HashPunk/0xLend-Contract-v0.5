...
describe('GovernorBravo#state/1', () => {
  let led, gov, root, acct, delay, timelock;
  ...
  beforeAll(async () => {
    ...
    gov = await deploy('GovernorBravoImmutable', [timelock._address, led._address, root, 17280, 1, "100000000000000000000000"]);
    await send(gov, '_initiate');
    await send(timelock, "harnessSetAdmin", [gov._address])
    await send(led, 'transfer', [acct, etherMantissa(4000000)]);
    await send(led, 'delegate', [acct], { from: acct });
  });
  ...
  it("Canceled", async () => {
    await send(led, 'transfer', [accounts[0], etherMantissa(4000000)]);
    await send(led, 'delegate', [accounts[0]], { from: accounts[0] });
    await mineBlock()
    ...
    await send(led, 'delegate', [root], { from: accounts[0] });

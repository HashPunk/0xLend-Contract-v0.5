...
describe('CompScenario', () => {
  ...
  beforeEach(async () => {
    ...
    led = await deploy('CompScenario', [root]);
  });
    ...
    [
      ...
      it(`with ${checkpoints} checkpoints, has ${expectedReads} reads`, async () => {
        ...
        while (remaining > 0) {
          ...
          await led.methods.generateCheckpoints(amt, offset).send({from: root, gas: 200000000});
          remaining -= amt;
          offset += amt;
        }

        let result = await led.methods.getPriorVotes(root, 1).send();

const {
  etherMantissa,
  both
} = require('../Utils/Ethereum');

const {
  makextroller,
  makePriceOracle,
  makextoken,
  makeToken
} = require('../Utils/0xlend');

describe('xtroller', () => {
  let root, accounts;

  beforeEach(async () => {
    [root, ...accounts] = saddle.accounts;
  });

  describe('constructor', () => {
    it("on success it sets admin to creator and pendingAdmin is unset", async () => {
      const xtroller = await makextroller();
      expect(await call(xtroller, 'admin')).toEqual(root);
      expect(await call(xtroller, 'pendingAdmin')).toEqualNumber(0);
    });

    it("on success it sets closeFactor as specified", async () => {
      const xtroller = await makextroller();
      expect(await call(xtroller, 'closeFactorMantissa')).toEqualNumber(0.051e18);
    });
  });

  describe('_setLiquidationIncentive', () => {
    const initialIncentive = etherMantissa(1.0);
    const validIncentive = etherMantissa(1.1);
    const tooSmallIncentive = etherMantissa(0.99999);
    const tooLargeIncentive = etherMantissa(1.50000001);

    let xtroller;
    beforeEach(async () => {
      xtroller = await makextroller();
    });

    it("fails if called by non-admin", async () => {
      const {reply, receipt} = await both(xtroller, '_setLiquidationIncentive', [initialIncentive], {from: accounts[0]});
      expect(reply).toHaveTrollError('UNAUTHORIZED');
      expect(receipt).toHaveTrollFailure('UNAUTHORIZED', 'SET_LIQUIDATION_INCENTIVE_OWNER_CHECK');
      expect(await call(xtroller, 'liquidationIncentiveMantissa')).toEqualNumber(initialIncentive);
    });

    it("accepts a valid incentive and emits a NewLiquidationIncentive event", async () => {
      const {reply, receipt} = await both(xtroller, '_setLiquidationIncentive', [validIncentive]);
      expect(reply).toHaveTrollError('NO_ERROR');
      expect(receipt).toHaveLog('NewLiquidationIncentive', {
        oldLiquidationIncentiveMantissa: initialIncentive.toString(),
        newLiquidationIncentiveMantissa: validIncentive.toString()
      });
      expect(await call(xtroller, 'liquidationIncentiveMantissa')).toEqualNumber(validIncentive);
    });
  });

  describe('_setPriceOracle', () => {
    let xtroller, oldOracle, newOracle;
    beforeEach(async () => {
      xtroller = await makextroller();
      oldOracle = xtroller.priceOracle;
      newOracle = await makePriceOracle();
    });

    it("fails if called by non-admin", async () => {
      expect(
        await send(xtroller, '_setPriceOracle', [newOracle._address], {from: accounts[0]})
      ).toHaveTrollFailure('UNAUTHORIZED', 'SET_PRICE_ORACLE_OWNER_CHECK');
      expect(await xtroller.methods.oracle().call()).toEqual(oldOracle._address);
    });

    it.skip("reverts if passed a contract that doesn't implement isPriceOracle", async () => {
      await expect(send(xtroller, '_setPriceOracle', [xtroller._address])).rejects.toRevert();
      expect(await call(xtroller, 'oracle')).toEqual(oldOracle._address);
    });

    it.skip("reverts if passed a contract that implements isPriceOracle as false", async () => {
      await send(newOracle, 'setIsPriceOracle', [false]); // Note: not yet implemented
      await expect(send(notOracle, '_setPriceOracle', [xtroller._address])).rejects.toRevert("revert oracle method isPriceOracle returned false");
      expect(await call(xtroller, 'oracle')).toEqual(oldOracle._address);
    });

    it("accepts a valid price oracle and emits a NewPriceOracle event", async () => {
      const result = await send(xtroller, '_setPriceOracle', [newOracle._address]);
      expect(result).toSucceed();
      expect(result).toHaveLog('NewPriceOracle', {
        oldPriceOracle: oldOracle._address,
        newPriceOracle: newOracle._address
      });
      expect(await call(xtroller, 'oracle')).toEqual(newOracle._address);
    });
  });

  describe('_setCloseFactor', () => {
    it("fails if not called by admin", async () => {
      const xtoken = await makextoken();
      await expect(
        send(xtoken.xtroller, '_setCloseFactor', [1], {from: accounts[0]})
      ).rejects.toRevert('revert only admin can set close factor');
    });
  });

  describe('_setCollateralFactor', () => {
    const half = etherMantissa(0.5);
    const one = etherMantissa(1);

    it("fails if not called by admin", async () => {
      const xtoken = await makextoken();
      expect(
        await send(xtoken.xtroller, '_setCollateralFactor', [xtoken._address, half], {from: accounts[0]})
      ).toHaveTrollFailure('UNAUTHORIZED', 'SET_COLLATERAL_FACTOR_OWNER_CHECK');
    });

    it("fails if asset is not listed", async () => {
      const xtoken = await makextoken();
      expect(
        await send(xtoken.xtroller, '_setCollateralFactor', [xtoken._address, half])
      ).toHaveTrollFailure('MARKET_NOT_LISTED', 'SET_COLLATERAL_FACTOR_NO_EXISTS');
    });

    it("fails if factor is set without an underlying price", async () => {
      const xtoken = await makextoken({supportMarket: true});
      expect(
        await send(xtoken.xtroller, '_setCollateralFactor', [xtoken._address, half])
      ).toHaveTrollFailure('PRICE_ERROR', 'SET_COLLATERAL_FACTOR_WITHOUT_PRICE');
    });

    it("succeeds and sets market", async () => {
      const xtoken = await makextoken({supportMarket: true, underlyingPrice: 1});
      const result = await send(xtoken.xtroller, '_setCollateralFactor', [xtoken._address, half]);
      expect(result).toHaveLog('NewCollateralFactor', {
        xtoken: xtoken._address,
        oldCollateralFactorMantissa: '0',
        newCollateralFactorMantissa: half.toString()
      });
    });
  });

  describe('_supportMarket', () => {
    it("fails if not called by admin", async () => {
      const xtoken = await makextoken(root);
      expect(
        await send(xtoken.xtroller, '_supportMarket', [xtoken._address], {from: accounts[0]})
      ).toHaveTrollFailure('UNAUTHORIZED', 'SUPPORT_MARKET_OWNER_CHECK');
    });

    it("fails if asset is not a xtoken", async () => {
      const xtroller = await makextroller()
      const asset = await makeToken(root);
      await expect(send(xtroller, '_supportMarket', [asset._address])).rejects.toRevert();
    });

    it("succeeds and sets market", async () => {
      const xtoken = await makextoken();
      const result = await send(xtoken.xtroller, '_supportMarket', [xtoken._address]);
      expect(result).toHaveLog('MarketListed', {xtoken: xtoken._address});
    });

    it("cannot list a market a second time", async () => {
      const xtoken = await makextoken();
      const result1 = await send(xtoken.xtroller, '_supportMarket', [xtoken._address]);
      const result2 = await send(xtoken.xtroller, '_supportMarket', [xtoken._address]);
      expect(result1).toHaveLog('MarketListed', {xtoken: xtoken._address});
      expect(result2).toHaveTrollFailure('MARKET_ALREADY_LISTED', 'SUPPORT_MARKET_EXISTS');
    });

    it("can list two different markets", async () => {
      const xtoken1 = await makextoken();
      const xtoken2 = await makextoken({xtroller: xtoken1.xtroller});
      const result1 = await send(xtoken1.xtroller, '_supportMarket', [xtoken1._address]);
      const result2 = await send(xtoken1.xtroller, '_supportMarket', [xtoken2._address]);
      expect(result1).toHaveLog('MarketListed', {xtoken: xtoken1._address});
      expect(result2).toHaveLog('MarketListed', {xtoken: xtoken2._address});
    });
  });

  describe('redeemVerify', () => {
    it('should allow you to redeem 0 underlying for 0 tokens', async () => {
      const xtroller = await makextroller();
      const xtoken = await makextoken({xtroller: xtroller});
      await call(xtroller, 'redeemVerify', [xtoken._address, accounts[0], 0, 0]);
    });

    it('should allow you to redeem 5 underlyig for 5 tokens', async () => {
      const xtroller = await makextroller();
      const xtoken = await makextoken({xtroller: xtroller});
      await call(xtroller, 'redeemVerify', [xtoken._address, accounts[0], 5, 5]);
    });

    it('should not allow you to redeem 5 underlying for 0 tokens', async () => {
      const xtroller = await makextroller();
      const xtoken = await makextoken({xtroller: xtroller});
      await expect(call(xtroller, 'redeemVerify', [xtoken._address, accounts[0], 5, 0])).rejects.toRevert("revert redeemTokens zero");
    });
  })
});

...
describe('PriceOracleProxy', () => {
  ...
  beforeEach(async () => {
    ...
    cEth = await makextoken({kind: "xkcc", xtrollerOpts: {kind: "v1-no-proxy"}, supportMarket: true});
    cUsdc = await makextoken({xtroller: cEth.xtroller, supportMarket: true});
    cSai = await makextoken({xtroller: cEth.xtroller, supportMarket: true});
    cDai = await makextoken({xtroller: cEth.xtroller, supportMarket: true});
    cUsdt = await makextoken({xtroller: cEth.xtroller, supportMarket: true});
    cOther = await makextoken({xtroller: cEth.xtroller, supportMarket: true});
  ...
  describe("constructor", () => {
    ...
    it("sets address of cEth", async () => {
      let configuredxkcc = await call(oracle, "cEthAddress");
      expect(configuredxkcc).toEqual(cEth._address);
    });
  ...
  describe("getUnderlyingPrice", () => {
    let setAndVerifyBackingPrice = async (xtoken, price) => {
      await send(
        backingOracle,
        "setUnderlyingPrice",
        [xtoken._address, etherMantissa(price)]);

      let backingOraclePrice = await call(
        backingOracle,
        "assetPrices",
        [xtoken.underlying._address]);
    ...
    it("returns 0 for token without a price", async () => {
      let unlistedToken = await makextoken({xtroller: cEth.xtroller});
